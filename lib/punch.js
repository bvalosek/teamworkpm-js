define(function(require) {
    ...
    .then(function(logs) {
        ...
        var lateTasks = _(tasks).filter(function(task) {
            var d = moment().diff(task.attributes.dateDue, 'days');

            console.log(task.attributes.dateDue.format('MM DD YYYY'));
        });

        console.log('My late tasks: ' + lateTasks.length);

        var loggedTasks = _(tasks).filter(function(task) {
            var entries = task.getTimeEntries();

            if (entries) {
                _(entries).each(function(entry) {
                    console.log(task.attributes.content);
                    console.log('  ' + entry.attributes.minutes + ' minutes');
                });
            }
        });
    });
